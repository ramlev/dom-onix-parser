<?php

namespace Dso\Onix\CodeList;

/**
 * ONIX Code List 96
 *
 * Used with    • <CurrencyCode> <j152>    • <DefaultCurrencyCode> <m186>
 *
 * @see https://ns.editeur.org/onix/en/96
 */
class CodeList96 extends CodeList implements CodeListInterface
{
	/**
	 * Code List 96 for en
	 *
	 * @var array
	 * @see https://ns.editeur.org/onix/en/96
	 */
	protected static $en = [
		'AED' => 'UAE Dirham',
		'AFA' => 'Afghani',
		'AFN' => 'Afghani',
		'ALL' => 'Lek',
		'AMD' => 'Armenian Dram',
		'ANG' => 'Netherlands Antillean Guilder',
		'AOA' => 'Kwanza',
		'ARS' => 'Argentine Peso',
		'ATS' => 'Schilling',
		'AUD' => 'Australian Dollar',
		'AWG' => 'Aruban Florin',
		'AZN' => 'Azerbaijan Manat',
		'BAM' => 'Convertible Marks',
		'BBD' => 'Barbados Dollar',
		'BDT' => 'Taka',
		'BEF' => 'Belgian Franc',
		'BGL' => 'Bulgarian Lev',
		'BGN' => 'Bulgarian Lev',
		'BHD' => 'Bahraini Dinar',
		'BIF' => 'Burundi Franc',
		'BMD' => 'Bermudian Dollar',
		'BND' => 'Brunei Dollar',
		'BOB' => 'Boliviano',
		'BRL' => 'Brazilian Real',
		'BSD' => 'Bahamian Dollar',
		'BTN' => 'Ngultrun',
		'BWP' => 'Pula',
		'BYR' => '(Old) Belarussian Ruble',
		'BYN' => 'Belarussian Ruble',
		'BZD' => 'Belize Dollar',
		'CAD' => 'Canadian Dollar',
		'CDF' => 'Franc Congolais',
		'CHF' => 'Swiss Franc',
		'CLP' => 'Chilean Peso',
		'CNY' => 'Yuan Renminbi',
		'COP' => 'Colombian Peso',
		'CRC' => 'Costa Rican Colon',
		'CSD' => 'Serbian Dinar',
		'CUC' => 'Cuban Convertible Peso',
		'CUP' => 'Cuban Peso',
		'CVE' => 'Cabo Verde Escudo',
		'CYP' => 'Cyprus Pound',
		'CZK' => 'Czech Koruna',
		'DEM' => 'Mark',
		'DJF' => 'Djibouti Franc',
		'DKK' => 'Danish Krone',
		'DOP' => 'Dominican Peso',
		'DZD' => 'Algerian Dinar',
		'EEK' => 'Kroon',
		'EGP' => 'Egyptian Pound',
		'ERN' => 'Nakfa',
		'ESP' => 'Peseta',
		'ETB' => 'Ethiopian Birr',
		'EUR' => 'Euro',
		'FIM' => 'Markka',
		'FJD' => 'Fiji Dollar',
		'FKP' => 'Falkland Islands Pound',
		'FRF' => 'Franc',
		'GBP' => 'Pound Sterling',
		'GEL' => 'Lari',
		'GHC' => 'Ghana Cedi',
		'GHS' => 'Ghana Cedi',
		'GIP' => 'Gibraltar Pound',
		'GMD' => 'Dalasi',
		'GNF' => 'Guinean Franc',
		'GRD' => 'Drachma',
		'GTQ' => 'Quetzal',
		'GWP' => 'Guinea-Bissau Peso',
		'GYD' => 'Guyana Dollar',
		'HKD' => 'Hong Kong Dollar',
		'HNL' => 'Lempira',
		'HRK' => 'Kuna',
		'HTG' => 'Gourde',
		'HUF' => 'Forint',
		'IDR' => 'Rupiah',
		'IEP' => 'Punt',
		'ILS' => 'New Israeli Sheqel',
		'INR' => 'Indian Rupee',
		'IQD' => 'Iraqi Dinar',
		'IRR' => 'Iranian Rial',
		'ISK' => 'Iceland Krona',
		'ITL' => 'Lira',
		'JMD' => 'Jamaican Dollar',
		'JOD' => 'Jordanian Dinar',
		'JPY' => 'Yen',
		'KES' => 'Kenyan Shilling',
		'KGS' => 'Som',
		'KHR' => 'Riel',
		'KMF' => 'Comorian Franc',
		'KPW' => 'North Korean Won',
		'KRW' => 'Won',
		'KWD' => 'Kuwaiti Dinar',
		'KYD' => 'Cayman Islands Dollar',
		'KZT' => 'Tenge',
		'LAK' => 'Lao Kip',
		'LBP' => 'Lebanese Pound',
		'LKR' => 'Sri Lanka Rupee',
		'LRD' => 'Liberian Dollar',
		'LSL' => 'Loti',
		'LTL' => 'Litus',
		'LUF' => 'Luxembourg Franc',
		'LVL' => 'Latvian Lats',
		'LYD' => 'Libyan Dinar',
		'MAD' => 'Moroccan Dirham',
		'MDL' => 'Moldovan Leu',
		'MGA' => 'Malagasy Ariary',
		'MGF' => 'Malagasy Franc',
		'MKD' => 'Denar',
		'MMK' => 'Kyat',
		'MNT' => 'Tugrik',
		'MOP' => 'Pataca',
		'MRO' => '(Old) Ouguiya',
		'MRU' => 'Ouguiya',
		'MTL' => 'Maltese Lira',
		'MUR' => 'Mauritius Rupee',
		'MVR' => 'Rufiyaa',
		'MWK' => 'Malawi Kwacha',
		'MXN' => 'Mexican Peso',
		'MYR' => 'Malaysian Ringgit',
		'MZN' => 'Mozambique Metical',
		'NAD' => 'Namibia Dollar',
		'NGN' => 'Naira',
		'NIO' => 'Cordoba Oro',
		'NLG' => 'Guilder',
		'NOK' => 'Norwegian Krone',
		'NPR' => 'Nepalese Rupee',
		'NZD' => 'New Zealand Dollar',
		'OMR' => 'Rial Omani',
		'PAB' => 'Balboa',
		'PEN' => 'Sol',
		'PGK' => 'Kina',
		'PHP' => 'Philippine Peso',
		'PKR' => 'Pakistan Rupee',
		'PLN' => 'Złoty',
		'PTE' => 'Escudo',
		'PYG' => 'Guarani',
		'QAR' => 'Qatari Rial',
		'ROL' => 'Romanian Old Leu',
		'RON' => 'Romanian Leu',
		'RSD' => 'Serbian Dinar',
		'RUB' => 'Russian Ruble',
		'RUR' => 'Russian Ruble',
		'RWF' => 'Rwanda Franc',
		'SAR' => 'Saudi Riyal',
		'SBD' => 'Solomon Islands Dollar',
		'SCR' => 'Seychelles Rupee',
		'SDD' => 'Sudanese Dinar',
		'SDG' => 'Sudanese Pound',
		'SEK' => 'Swedish Krona',
		'SGD' => 'Singapore Dollar',
		'SHP' => 'Saint Helena Pound',
		'SIT' => 'Tolar',
		'SKK' => 'Slovak Koruna',
		'SLE' => 'Leone',
		'SLL' => 'Leone',
		'SOS' => 'Somali Shilling',
		'SRD' => 'Surinam Dollar',
		'SRG' => 'Suriname Guilder',
		'STD' => '(Old) Dobra',
		'STN' => 'Dobra',
		'SVC' => 'El Salvador Colon',
		'SYP' => 'Syrian Pound',
		'SZL' => 'Lilangeni',
		'THB' => 'Baht',
		'TJS' => 'Somoni',
		'TMM' => 'Turkmenistan Manat',
		'TMT' => 'Turkmenistan New Manat',
		'TND' => 'Tunisian Dinar',
		'TOP' => 'Pa’anga',
		'TPE' => 'Timor Escudo',
		'TRL' => 'Turkish Lira (old)',
		'TRY' => 'Turkish Lira',
		'TTD' => 'Trinidad and Tobago Dollar',
		'TWD' => 'New Taiwan Dollar',
		'TZS' => 'Tanzanian Shilling',
		'UAH' => 'Hryvnia',
		'UGX' => 'Uganda Shilling',
		'USD' => 'US Dollar',
		'UYU' => 'Peso Uruguayo',
		'UZS' => 'Uzbekistan Sum',
		'VEB' => 'Bolívar',
		'VEF' => 'Bolívar',
		'VES' => 'Bolívar Soberano',
		'VND' => 'Dong',
		'VUV' => 'Vatu',
		'WST' => 'Tala',
		'XAF' => 'CFA Franc BEAC',
		'XCD' => 'East Caribbean Dollar',
		'XCG' => 'Caribbean Guilder',
		'XOF' => 'CFA Franc BCEAO',
		'XPF' => 'CFP Franc',
		'YER' => 'Yemeni Rial',
		'YUM' => 'Yugoslavian Dinar',
		'ZAR' => 'Rand',
		'ZMK' => 'Kwacha',
		'ZMW' => 'Zambian Kwacha',
		'ZWD' => 'Zimbabwe Dollar',
		'ZWG' => 'Zimbabwe Gold',
		'ZWL' => 'Zimbabwe Dollar',
	];

	/**
	 * Code List 96 for es
	 *
	 * @var array
	 * @see https://ns.editeur.org/onix/es/96
	 */
	protected static $es = [
		'AED' => 'Dírham de los Emiratos Árabes Unidos',
		'AFA' => 'Afgani afgano',
		'AFN' => 'Afgani afgano',
		'ALL' => 'Lek albano',
		'AMD' => 'Dram armenio',
		'ANG' => 'Florín de las Antillas Neerlandesas',
		'AOA' => 'Kwanza angoleño',
		'ARS' => 'Peso argentino',
		'ATS' => 'Chelín austriaco',
		'AUD' => 'Dólar australiano',
		'AWG' => 'Guilder de Aruba',
		'AZN' => 'Manat azerbaiyano',
		'BAM' => 'Marco convertible de Bosnia-Herzegovina',
		'BBD' => 'Dólar de Barbados',
		'BDT' => 'Taka de Bangla Desh',
		'BEF' => 'Franco belga',
		'BGL' => 'Lev búlgaro',
		'BGN' => 'Lev búlgaro',
		'BHD' => 'Dinar bahreiní',
		'BIF' => 'Franco burundés',
		'BMD' => 'Dólar de Bermuda',
		'BND' => 'Dólar de Brunei',
		'BOB' => 'Boliviano',
		'BRL' => 'Real brasileño',
		'BSD' => 'Dólar bahameño',
		'BTN' => 'Ngultrum de Bután',
		'BWP' => 'Pula de Botswana',
		'BYR' => 'Rublo bielorruso',
		'BYN' => 'Rublo bielorruso',
		'BZD' => 'Dólar de Belice',
		'CAD' => 'Dólar canadiense',
		'CDF' => 'Franco congoleño',
		'CHF' => 'Franco suizo',
		'CLP' => 'Peso chileno',
		'CNY' => 'Yuan Renminbi',
		'COP' => 'Peso colombiano',
		'CRC' => 'Colón costarricense',
		'CSD' => 'Dinar serbio',
		'CUC' => 'Peso convertible cubano',
		'CUP' => 'Peso cubano',
		'CVE' => 'Escudo caboverdiano',
		'CYP' => 'Libra chipriota',
		'CZK' => 'Corona checa',
		'DEM' => 'Marco alemán',
		'DJF' => 'Franco yibutiano',
		'DKK' => 'Corona danesa',
		'DOP' => 'Peso dominicano',
		'DZD' => 'Dinar algerino',
		'EEK' => 'Corona estonia',
		'EGP' => 'Libra egipcia',
		'ERN' => 'Nakfa eritreo',
		'ESP' => 'Peseta española',
		'ETB' => 'Birr etíope',
		'EUR' => 'Euro',
		'FIM' => 'Marco finlandés',
		'FJD' => 'Dólar fijiano',
		'FKP' => 'Libra malvinense',
		'FRF' => 'Franco francés',
		'GBP' => 'Libra esterlina',
		'GEL' => 'Lari georgiano',
		'GHC' => 'Cedi ghanés',
		'GHS' => 'Cedi',
		'GIP' => 'Libra de Gibraltar',
		'GMD' => 'Dalasi gambiano',
		'GNF' => 'Franco guineano',
		'GRD' => 'Dracma griega',
		'GTQ' => 'Quetzal guatemalteco',
		'GWP' => 'Peso de Guiena-Bissau',
		'GYD' => 'Dólar guyanés',
		'HKD' => 'Dólar de Hong Kong',
		'HNL' => 'Lempira hondureña',
		'HRK' => 'Kuna croata',
		'HTG' => 'Gourde haitiano',
		'HUF' => 'Forint húngaro',
		'IDR' => 'Rupia indonesia',
		'IEP' => 'Libra irlandesa',
		'ILS' => 'Nuevo shequel israelí',
		'INR' => 'Rupia india',
		'IQD' => 'Dinar iraqí',
		'IRR' => 'Rial iraní',
		'ISK' => 'Króna islandesa',
		'ITL' => 'Lira italiana',
		'JMD' => 'Dólar jamaicano',
		'JOD' => 'Dinar jordano',
		'JPY' => 'Yen japonés',
		'KES' => 'Chelín keniata',
		'KGS' => 'Som kirguís (de Kirguistán)',
		'KHR' => 'Riel camboyano',
		'KMF' => 'Franco comoriano (de Comoras)',
		'KPW' => 'Won norcoreano',
		'KRW' => 'Won surcoreano',
		'KWD' => 'Dinar kuwaití',
		'KYD' => 'Dólar caimano (de Islas Caimán)',
		'KZT' => 'Tenge kazajo (de Kazajstán)',
		'LAK' => 'Kip lao',
		'LBP' => 'Libra libanesa',
		'LKR' => 'Rupia de Sri Lanka',
		'LRD' => 'Dólar liberiano',
		'LSL' => 'Loti lesothense (de Lesotho)',
		'LTL' => 'Litas lituano',
		'LUF' => 'Franco luxemburgués',
		'LVL' => 'Lat letón',
		'LYD' => 'Dinar libio',
		'MAD' => 'Dirham marroquí',
		'MDL' => 'Leu moldavo',
		'MGA' => 'Ariary malgache',
		'MGF' => 'Franco malgache',
		'MKD' => 'Denar macedonio',
		'MMK' => 'Kyat myanmaro',
		'MNT' => 'Tughrik mongol',
		'MOP' => 'Pataca de Macao',
		'MRO' => 'Ouguiya mauritana (Antigua)',
		'MRU' => 'Ouguiya mauritana',
		'MTL' => 'Lira maltesa',
		'MUR' => 'Rupia mauricia',
		'MVR' => 'Rufiyaa maldiva',
		'MWK' => 'Kwacha malawiano',
		'MXN' => 'Peso mexicano',
		'MYR' => 'Ringgit malayo',
		'MZN' => 'Metical mozambiqueño',
		'NAD' => 'Dólar namibio',
		'NGN' => 'Naira nigeriana',
		'NIO' => 'Córdoba nicaragüense',
		'NLG' => 'Guilder holandés',
		'NOK' => 'Corona noruega',
		'NPR' => 'Rupia nepalesa',
		'NZD' => 'Dólar neozelandés',
		'OMR' => 'Rial omaní',
		'PAB' => 'Balboa panameña',
		'PEN' => 'Nuevo sol peruano',
		'PGK' => 'Kina de Papúa Nueva Guinea',
		'PHP' => 'Peso filipino',
		'PKR' => 'Rupia pakistaní',
		'PLN' => 'zloty polaco',
		'PTE' => 'Escudo portugués',
		'PYG' => 'Guaraní paraguayo',
		'QAR' => 'Rial qatarí',
		'ROL' => 'Leu rumano',
		'RON' => 'Nuevo Leu rumano',
		'RSD' => 'Dinar serbio',
		'RUB' => 'Rublo ruso',
		'RUR' => 'Rublo ruso',
		'RWF' => 'Franco ruandés',
		'SAR' => 'Riyal saudí',
		'SBD' => 'Dólar de las Islas Salomón',
		'SCR' => 'Rupia de Seychelles',
		'SDD' => 'Dinar sudanés',
		'SDG' => 'Libra sudanesa',
		'SEK' => 'Corona sueca',
		'SGD' => 'Dólar de Singapur',
		'SHP' => 'Libra de Santa Helena',
		'SIT' => 'Tólar esloveno',
		'SKK' => 'Corona eslovaca',
		'SLE' => 'Leone',
		'SLL' => 'Leone',
		'SOS' => 'Chelín somalí',
		'SRD' => 'Dólar surinamés (desde el 1 de enero de 2004)',
		'SRG' => 'Dólar surinamés',
		'STD' => 'Dobra santotomense (Antigua)',
		'STN' => 'Dobra santotomense',
		'SVC' => 'Colón de El Salvador',
		'SYP' => 'Libra siria',
		'SZL' => 'Lilangeni',
		'THB' => 'Baht tailandés',
		'TJS' => 'Rublo tayik (de Tayikistán)',
		'TMM' => 'Manat turcomano',
		'TMT' => 'Manat',
		'TND' => 'Dinar tunecino',
		'TOP' => 'Pa’anga tongano',
		'TPE' => 'Escudo de Timor',
		'TRL' => 'Antigua lira turca',
		'TRY' => 'Nueva lira turca',
		'TTD' => 'Dólar de Trinidad y Tobago',
		'TWD' => 'Dólar taiwanés',
		'TZS' => 'Chelín tanzano',
		'UAH' => 'Hryvnia ucraniana',
		'UGX' => 'Chelín ugandés',
		'USD' => 'Dólar estadounidense',
		'UYU' => 'Peso uruguayo',
		'UZS' => 'Som uzbeco',
		'VEB' => 'Bolívar venezolano',
		'VEF' => 'Bolívar',
		'VES' => 'Bolívar soberano',
		'VND' => 'Dong vietnamita',
		'VUV' => 'Vatu vanuatense',
		'WST' => 'Tala samoana',
		'XAF' => 'Franco CFA BEAC',
		'XCD' => 'Dólar del Caribe Oriental',
		'XCG' => 'Guilder del cáribe',
		'XOF' => 'Franco CFA BCEAO',
		'XPF' => 'Franco CFP',
		'YER' => 'Rial yemení',
		'YUM' => 'Dinar yugoslavo',
		'ZAR' => 'Rand sudafricano',
		'ZMK' => 'Kwacha zambiano',
		'ZMW' => 'Kwacha zambiano',
		'ZWD' => 'Dólar zimbabwés',
		'ZWG' => 'Zimbabwe Gold',
		'ZWL' => 'Dólar zimbabwés',
	];

	/**
	 * Code List 96 for de
	 *
	 * @var array
	 * @see https://ns.editeur.org/onix/de/96
	 */
	protected static $de = [
		'AED' => 'VAE-Dirham',
		'AFA' => 'Afghani',
		'AFN' => 'Afghani',
		'ALL' => 'Lek',
		'AMD' => 'Armenischer Dram',
		'ANG' => 'Niederländischer Antillen-Gulden',
		'AOA' => 'Kwanza',
		'ARS' => 'Argentinischer Peso',
		'ATS' => 'Schilling',
		'AUD' => 'Australischer Dollar',
		'AWG' => 'Aruba-Florin',
		'AZN' => 'Aserbaidschan-Manat',
		'BAM' => 'Konvertible Mark',
		'BBD' => 'Barbados-Dollar',
		'BDT' => 'Taka',
		'BEF' => 'Belgischer Franc',
		'BGL' => 'Bulgarischer Lew',
		'BGN' => 'Bulgarischer Lew',
		'BHD' => 'Bahraini-Dinar',
		'BIF' => 'Burundi-Franc',
		'BMD' => 'Bermudadollar',
		'BND' => 'Brunei-Dollar',
		'BOB' => 'Boliviano',
		'BRL' => 'Brasilianischer Real',
		'BSD' => 'Bahama-Dollar',
		'BTN' => 'Ngultrum',
		'BWP' => 'Pula',
		'BYR' => '(Alter) Belarussischer Rubel',
		'BYN' => 'Belarussischer Rubel',
		'BZD' => 'Belize-Dollar',
		'CAD' => 'Kanadischer Dollar',
		'CDF' => 'Kongolesischer Franc',
		'CHF' => 'Schweizer Franken',
		'CLP' => 'Chilenischer Peso',
		'CNY' => 'Renminbi Yuan',
		'COP' => 'Kolumbianischer Peso',
		'CRC' => 'Costa-Rica-Colón',
		'CSD' => 'Serbischer Dinar',
		'CUC' => 'Kubanischer konvertibler Peso',
		'CUP' => 'Kubanischer Peso',
		'CVE' => 'Kap-Verde-Escudo',
		'CYP' => 'Zypern-Pfund',
		'CZK' => 'Tschechische Krone',
		'DEM' => 'Mark',
		'DJF' => 'Dschibuti-Franc',
		'DKK' => 'Dänische Krone',
		'DOP' => 'Dominikanischer Peso',
		'DZD' => 'Algerischer Dinar',
		'EEK' => 'Krone',
		'EGP' => 'Ägyptisches Pfund',
		'ERN' => 'Nakfa',
		'ESP' => 'Peseta',
		'ETB' => 'Äthiopischer Birr',
		'EUR' => 'Euro',
		'FIM' => 'Markka',
		'FJD' => 'Fidschi-Dollar',
		'FKP' => 'Falkland-Pfund',
		'FRF' => 'Franc',
		'GBP' => 'Pfund Sterling',
		'GEL' => 'Lari',
		'GHC' => 'Ghanaischer Cedi',
		'GHS' => 'Ghanaischer Cedi',
		'GIP' => 'Gibraltar-Pfund',
		'GMD' => 'Dalasi',
		'GNF' => 'Guinea-Franc',
		'GRD' => 'Drachme',
		'GTQ' => 'Quetzal',
		'GWP' => 'Guinea-Bissau-Peso',
		'GYD' => 'Guyana-Dollar',
		'HKD' => 'Hongkong-Dollar',
		'HNL' => 'Lempira',
		'HRK' => 'Kuna',
		'HTG' => 'Gourde',
		'HUF' => 'Forint',
		'IDR' => 'Rupiah',
		'IEP' => 'Punt',
		'ILS' => 'Neuer Israelischer Schekel',
		'INR' => 'Indische Rupie',
		'IQD' => 'Irakischer Dinar',
		'IRR' => 'Iranischer Rial',
		'ISK' => 'Isländische Krone',
		'ITL' => 'Lira',
		'JMD' => 'Jamaika-Dollar',
		'JOD' => 'Jordanischer Dinar',
		'JPY' => 'Yen',
		'KES' => 'Kenianischer Schilling',
		'KGS' => 'Som',
		'KHR' => 'Riel',
		'KMF' => 'Komoren-Franc',
		'KPW' => 'Nordkoreanischer Won',
		'KRW' => 'Südkoreanischer Won',
		'KWD' => 'Kuwait-Dinar',
		'KYD' => 'Kaiman-Dollar',
		'KZT' => 'Tenge',
		'LAK' => 'Kip',
		'LBP' => 'Libanesisches Pfund',
		'LKR' => 'Sri-Lanka-Rupie',
		'LRD' => 'Liberianischer Dollar',
		'LSL' => 'Loti',
		'LTL' => 'Litas',
		'LUF' => 'Luxemburgischer Franc',
		'LVL' => 'Lettischer Lats',
		'LYD' => 'Libyscher Dinar',
		'MAD' => 'Marokkanischer Dirham',
		'MDL' => 'Moldauischer Leu',
		'MGA' => 'Ariary',
		'MGF' => 'Madagaskar-Franc',
		'MKD' => 'Denar',
		'MMK' => 'Kyat',
		'MNT' => 'Tugrik',
		'MOP' => 'Pataca',
		'MRO' => '(Alter) Ouguiya',
		'MRU' => 'Ouguiya',
		'MTL' => 'Maltesische Lira',
		'MUR' => 'Mauritius-Rupie',
		'MVR' => 'Rufiyaa',
		'MWK' => 'Malawi-Kwacha',
		'MXN' => 'Mexikanischer Peso',
		'MYR' => 'Malaysischer Ringgit',
		'MZN' => 'Mosambik-Metical',
		'NAD' => 'Namibia-Dollar',
		'NGN' => 'Naira',
		'NIO' => 'Cordoba Oro',
		'NLG' => 'Gulden',
		'NOK' => 'Norwegische Krone',
		'NPR' => 'Nepalesische Rupie',
		'NZD' => 'Neuseeland-Dollar',
		'OMR' => 'Oman-Rial',
		'PAB' => 'Balboa',
		'PEN' => 'Sol',
		'PGK' => 'Kina',
		'PHP' => 'Philippinischer Peso',
		'PKR' => 'Pakistanische Rupie',
		'PLN' => 'Zloty',
		'PTE' => 'Escudo',
		'PYG' => 'Guarani',
		'QAR' => 'Katar-Riyal',
		'ROL' => 'Alter Rumänischer Leu',
		'RON' => 'Rumänischer Leu',
		'RSD' => 'Serbischer Dinar',
		'RUB' => 'Russischer Rubel',
		'RUR' => 'Russischer Rubel',
		'RWF' => 'Ruanda-Franc',
		'SAR' => 'Saudi-Riyal',
		'SBD' => 'Salomonen-Dollar',
		'SCR' => 'Seychellen-Rupie',
		'SDD' => 'Sudanesischer Dinar',
		'SDG' => 'Sudanesisches Pfund',
		'SEK' => 'Schwedische Krone',
		'SGD' => 'Singapur-Dollar',
		'SHP' => 'St.-Helena-Pfund',
		'SIT' => 'Tolar',
		'SKK' => 'Slowakische Krone',
		'SLE' => 'Leone (neu)',
		'SLL' => 'Leone (alt)',
		'SOS' => 'Somali-Schilling',
		'SRD' => 'Suriname-Dollar',
		'SRG' => 'Suriname-Gulden',
		'STD' => '(Alter) Dobra',
		'STN' => 'Dobra',
		'SVC' => 'El-Salvador-Colón',
		'SYP' => 'Syrisches Pfund',
		'SZL' => 'Lilangeni',
		'THB' => 'Baht',
		'TJS' => 'Somoni',
		'TMM' => 'Turkmenistan-Manat (alt)',
		'TMT' => 'Neuer Turkmenistan-Manat',
		'TND' => 'Tunesischer Dinar',
		'TOP' => 'Pa’anga',
		'TPE' => 'Timor-Escudo',
		'TRL' => 'Türkische Lira (alt)',
		'TRY' => 'Türkische Lira',
		'TTD' => 'Trinidad-und-Tobago-Dollar',
		'TWD' => 'Neuer Taiwan-Dollar',
		'TZS' => 'Tansania-Schilling',
		'UAH' => 'Hrywnja',
		'UGX' => 'Uganda-Schilling',
		'USD' => 'US-Dollar',
		'UYU' => 'Uruguayanischer Peso',
		'UZS' => 'Usbekistan-Sum',
		'VEB' => 'Bolívar',
		'VEF' => 'Bolívar (Fuerte/veraltet)',
		'VES' => 'Bolívar Soberano',
		'VND' => 'Dong',
		'VUV' => 'Vatu',
		'WST' => 'Tala',
		'XAF' => 'CFA-Franc BEAC',
		'XCD' => 'Ostkaribischer Dollar',
		'XCG' => 'Karibischer Gulden',
		'XOF' => 'CFA-Franc BCEAO',
		'XPF' => 'CFP-Franc',
		'YER' => 'Jemen-Rial',
		'YUM' => 'Jugoslawischer Dinar',
		'ZAR' => 'Rand',
		'ZMK' => 'Kwacha',
		'ZMW' => 'Sambia-Kwacha',
		'ZWD' => 'Simbabwe-Dollar',
		'ZWG' => 'Simbabwe-Gold',
		'ZWL' => 'Simbabwe-Dollar',
	];

	/**
	 * Code List 96 for fr
	 *
	 * @var array
	 * @see https://ns.editeur.org/onix/fr/96
	 */
	protected static $fr = [
		'AED' => 'Dirham',
		'AFA' => 'Afghani',
		'AFN' => 'Afghani',
		'ALL' => 'Lek',
		'AMD' => 'Dram',
		'ANG' => 'Florin (Antilles néerlandaises)',
		'AOA' => 'Kwanza',
		'ARS' => 'Peso',
		'ATS' => 'Shilling',
		'AUD' => 'Dollar australien',
		'AWG' => 'Florin',
		'AZN' => 'Manat azerbaïdjanais',
		'BAM' => 'Mark convertible',
		'BBD' => 'Dollar',
		'BDT' => 'Taka',
		'BEF' => 'Franc belge',
		'BGL' => 'Lev bulgare',
		'BGN' => 'Lev',
		'BHD' => 'Dinar',
		'BIF' => 'Franc',
		'BMD' => 'Dollar',
		'BND' => 'Dollar',
		'BOB' => 'Boliviano',
		'BRL' => 'Réal',
		'BSD' => 'Dollar',
		'BTN' => 'Ngultrum',
		'BWP' => 'Pula',
		'BYR' => 'Rouble',
		'BYN' => 'Rouble biélorusse',
		'BZD' => 'Dollar',
		'CAD' => 'Dollar',
		'CDF' => 'Franc',
		'CHF' => 'Franc',
		'CLP' => 'Peso',
		'CNY' => 'Yuan renminbi',
		'COP' => 'Peso',
		'CRC' => 'Colon',
		'CSD' => 'Dinar serbe',
		'CUC' => 'Peso cubain convertible',
		'CUP' => 'Peso',
		'CVE' => 'Escudo',
		'CYP' => 'Livre de Chypre',
		'CZK' => 'Couronne',
		'DEM' => 'Mark',
		'DJF' => 'Franc',
		'DKK' => 'Couronne',
		'DOP' => 'Peso',
		'DZD' => 'Dinar',
		'EEK' => 'Couronne',
		'EGP' => 'Livre',
		'ERN' => 'Nakfa',
		'ESP' => 'Peseta',
		'ETB' => 'Birr',
		'EUR' => 'Euro',
		'FIM' => 'Mark finlandais',
		'FJD' => 'Dollar',
		'FKP' => 'Livre',
		'FRF' => 'Franc',
		'GBP' => 'Livre sterling',
		'GEL' => 'Lari',
		'GHC' => 'Cedi ghanéen',
		'GHS' => 'Cedi',
		'GIP' => 'Livre',
		'GMD' => 'Dalasi',
		'GNF' => 'Franc guinéen',
		'GRD' => 'Drachme',
		'GTQ' => 'Quetzal',
		'GWP' => 'Peso de Guinée Bissau',
		'GYD' => 'Dollar',
		'HKD' => 'Dollar',
		'HNL' => 'Lempira',
		'HRK' => 'Kuna',
		'HTG' => 'Gourde',
		'HUF' => 'Forint',
		'IDR' => 'Roupie',
		'IEP' => 'Livre irlandaise',
		'ILS' => 'Shekel',
		'INR' => 'Roupie',
		'IQD' => 'Dinar',
		'IRR' => 'Rial',
		'ISK' => 'Couronne',
		'ITL' => 'Lire',
		'JMD' => 'Dollar',
		'JOD' => 'Dinar',
		'JPY' => 'Yen',
		'KES' => 'Shilling',
		'KGS' => 'Som',
		'KHR' => 'Riel',
		'KMF' => 'Franc comorien',
		'KPW' => 'Won',
		'KRW' => 'Won',
		'KWD' => 'Dinar',
		'KYD' => 'Dollar',
		'KZT' => 'Tenge',
		'LAK' => 'Kip du Laos',
		'LBP' => 'Livre',
		'LKR' => 'Roupie',
		'LRD' => 'Dollar',
		'LSL' => 'Loti',
		'LTL' => 'Litus',
		'LUF' => 'Franc luxembourgeois',
		'LVL' => 'Lats',
		'LYD' => 'Dinar',
		'MAD' => 'Dirham',
		'MDL' => 'Leu',
		'MGA' => 'Ariary',
		'MGF' => 'France magache',
		'MKD' => 'Denar',
		'MMK' => 'Kyat',
		'MNT' => 'Tugrik',
		'MOP' => 'Pataca',
		'MRO' => 'Ouguiya (ancien)',
		'MRU' => 'Ouguiya',
		'MTL' => 'Lire maltaise',
		'MUR' => 'Roupie',
		'MVR' => 'Rufiyaa',
		'MWK' => 'Kwacha',
		'MXN' => 'Peso',
		'MYR' => 'Ringgit',
		'MZN' => 'Metical',
		'NAD' => 'Dollar',
		'NGN' => 'Naira',
		'NIO' => 'Cordoba Oro',
		'NLG' => 'Florin',
		'NOK' => 'Couronne',
		'NPR' => 'Roupie',
		'NZD' => 'Dollar',
		'OMR' => 'Rial',
		'PAB' => 'Balboa',
		'PEN' => 'Nuevo Sol',
		'PGK' => 'Kina',
		'PHP' => 'Peso philippin',
		'PKR' => 'Roupie',
		'PLN' => 'Zloty',
		'PTE' => 'Escudo',
		'PYG' => 'Guarani',
		'QAR' => 'Rial',
		'ROL' => 'Leu roumain',
		'RON' => 'Leu (nouveau)',
		'RSD' => 'Dinar',
		'RUB' => 'Rouble',
		'RUR' => 'Rouble',
		'RWF' => 'Franc',
		'SAR' => 'Riyal',
		'SBD' => 'Dollar',
		'SCR' => 'Roupie',
		'SDD' => 'Dinar soudanais',
		'SDG' => 'Livre',
		'SEK' => 'Couronne',
		'SGD' => 'Dollar',
		'SHP' => 'Livre',
		'SIT' => 'Tolar',
		'SKK' => 'Couronne slovaque',
		'SLE' => 'Leone',
		'SLL' => 'Leone',
		'SOS' => 'Shilling',
		'SRD' => 'Florin',
		'SRG' => 'Florin du Surinam',
		'STD' => 'Dobra (ancien)',
		'STN' => 'Dobra',
		'SVC' => 'Colon',
		'SYP' => 'Livre',
		'SZL' => 'Lilangeni',
		'THB' => 'Baht',
		'TJS' => 'Somoni',
		'TMM' => 'Manat',
		'TMT' => 'Manat',
		'TND' => 'Dinar',
		'TOP' => 'Paʼanga',
		'TPE' => 'Escudo du Timor-vert',
		'TRL' => 'Lire turque (ancien)',
		'TRY' => 'Livre (nouvelle)',
		'TTD' => 'Dollar',
		'TWD' => 'Dollar',
		'TZS' => 'Shilling',
		'UAH' => 'Hryvnia',
		'UGX' => 'Shilling',
		'USD' => 'Dollar',
		'UYU' => 'Peso',
		'UZS' => 'Sum',
		'VEB' => 'Bolivar',
		'VEF' => 'Bolivar',
		'VES' => 'Bolivar Soberano',
		'VND' => 'Dong',
		'VUV' => 'Vatu',
		'WST' => 'Tala',
		'XAF' => 'Franc CFA (BEAC)',
		'XCD' => 'Dollar (OECO)',
		'XCG' => 'Florin caribéen',
		'XOF' => 'Franc CFA (BCEAO)',
		'XPF' => 'Franc CFP',
		'YER' => 'Rial',
		'YUM' => 'Dinar yougoslave',
		'ZAR' => 'Rand',
		'ZMK' => 'Kwacha',
		'ZMW' => 'Kwacha zambien',
		'ZWD' => 'Dollar du Zimbabwe',
		'ZWG' => 'Or zimbabwéen',
		'ZWL' => 'Dollar',
	];

	/**
	 * Code List 96 for it
	 *
	 * @var array
	 * @see https://ns.editeur.org/onix/it/96
	 */
	protected static $it = [
		'AED' => 'Dirham degli Emirati Arabi Uniti',
		'AFA' => 'Afghani',
		'AFN' => 'Afghani',
		'ALL' => 'Lek albanese',
		'AMD' => 'Dram armeno',
		'ANG' => 'Fiorino delle Antille olandesi',
		'AOA' => 'Kwanza angolano',
		'ARS' => 'Peso argentino',
		'ATS' => 'Scellino austriaco',
		'AUD' => 'Dollaro australiano',
		'AWG' => 'Fiorino arubano',
		'AZN' => 'Manat azero',
		'BAM' => 'Marco bosniaco',
		'BBD' => 'Dollaro di Barbados',
		'BDT' => 'Taka bengalese',
		'BEF' => 'Franco belga',
		'BGL' => 'Lev',
		'BGN' => 'Nuovo lev bulgaro',
		'BHD' => 'Dinaro del Bahrain',
		'BIF' => 'Franco del Burundi',
		'BMD' => 'Dollaro della Bermuda',
		'BND' => 'Dollaro del Brunei',
		'BOB' => 'Boliviano boliviano',
		'BRL' => 'Real brasiliano',
		'BSD' => 'Dollaro delle Bahamas',
		'BTN' => 'Ngultrum del Bhutan',
		'BWP' => 'Pula del Botswana',
		'BYR' => 'Rublo bielorusso',
		'BYN' => 'Rublo bielorusso',
		'BZD' => 'Dollaro del Belize',
		'CAD' => 'Dollaro canadese',
		'CDF' => 'Franco congolese',
		'CHF' => 'Franco svizzero',
		'CLP' => 'Peso cileno',
		'CNY' => 'Renminbi cinese (Yuan)',
		'COP' => 'Peso colombiano',
		'CRC' => 'Colón costaricano',
		'CSD' => 'Dinaro serbo',
		'CUC' => 'Peso cubano convertibile',
		'CUP' => 'Peso cubano',
		'CVE' => 'Escudo di Capo Verde',
		'CYP' => 'Sterlina cipriota',
		'CZK' => 'Corona ceca',
		'DEM' => 'Marco tedesco',
		'DJF' => 'Franco gibutiano',
		'DKK' => 'Corona danese',
		'DOP' => 'Peso dominicano',
		'DZD' => 'Dinaro algerino',
		'EEK' => 'Corona',
		'EGP' => 'Lira egiziana (o sterlina)',
		'ERN' => 'Nakfa eritreo',
		'ESP' => 'Peseta spagnola',
		'ETB' => 'Birr etiope',
		'EUR' => 'Euro',
		'FIM' => 'Marco finlandese',
		'FJD' => 'Dollaro delle Figi',
		'FKP' => 'Sterlina delle Falkland',
		'FRF' => 'Franco francese',
		'GBP' => 'Sterlina britannica (o lira sterlina)',
		'GEL' => 'Lari georgiano',
		'GHC' => 'Cedi ghanese',
		'GHS' => 'Cedi ghanese',
		'GIP' => 'Sterlina di Gibilterra',
		'GMD' => 'Dalasi gambese',
		'GNF' => 'Franco guineano',
		'GRD' => 'Dracma greca',
		'GTQ' => 'Quetzal guatemalteco',
		'GWP' => 'Peso della Guinea-Bissau',
		'GYD' => 'Dollaro della Guyana',
		'HKD' => 'Dollaro di Hong Kong',
		'HNL' => 'Lempira honduregna',
		'HRK' => 'Kuna croata',
		'HTG' => 'Gourde haitiano',
		'HUF' => 'Fiorino ungherese',
		'IDR' => 'Rupia indonesiana',
		'IEP' => 'Sterlina irlandese',
		'ILS' => 'Nuovo siclo israeliano',
		'INR' => 'Rupia indiana',
		'IQD' => 'Dinaro iracheno',
		'IRR' => 'Rial iraniano',
		'ISK' => 'Corona islandese',
		'ITL' => 'Lira italiana',
		'JMD' => 'Dollaro giamaicano',
		'JOD' => 'Dinaro giordano',
		'JPY' => 'Yen giapponese',
		'KES' => 'Scellino keniota',
		'KGS' => 'Som kirghizo',
		'KHR' => 'Riel cambogiano',
		'KMF' => 'Franco delle Comore',
		'KPW' => 'Won nordcoreano',
		'KRW' => 'Won sudcoreano',
		'KWD' => 'Dinaro kuwaitiano',
		'KYD' => 'Dollaro delle Cayman',
		'KZT' => 'Tenge kazako',
		'LAK' => 'Kip laotiano',
		'LBP' => 'Lira libanese (o sterlina)',
		'LKR' => 'Rupia singalese',
		'LRD' => 'Dollaro liberiano',
		'LSL' => 'Loti lesothiano',
		'LTL' => 'Litas lituano',
		'LUF' => 'Franco (Lussemburgo)',
		'LVL' => 'Lats lettone',
		'LYD' => 'Dinaro libico',
		'MAD' => 'Dirham marocchino',
		'MDL' => 'Leu moldavo',
		'MGA' => 'Ariary malgascio',
		'MGF' => 'Franco malgascio',
		'MKD' => 'Dinaro macedone',
		'MMK' => 'Kyat birmano',
		'MNT' => 'Tugrik mongolo',
		'MOP' => 'Pataca di Macao',
		'MRO' => '(Vecchia) Ouguiya mauritana',
		'MRU' => 'Ouguiya mauritana',
		'MTL' => 'Lira maltese',
		'MUR' => 'Rupia mauriziana',
		'MVR' => 'Rufiyaa delle Maldive',
		'MWK' => 'Kwacha malawiano',
		'MXN' => 'Peso messicano',
		'MYR' => 'Ringgit malese',
		'MZN' => 'Metical mozambicano',
		'NAD' => 'Dollaro namibiano',
		'NGN' => 'Naira nigeriana',
		'NIO' => 'Cordoba nicaraguense',
		'NLG' => 'Fiorino (Paesi Bassi)',
		'NOK' => 'Corona norvegese',
		'NPR' => 'Rupia nepalese',
		'NZD' => 'Dollaro neozelandese',
		'OMR' => 'Rial dell’Oman',
		'PAB' => 'Balboa panamense',
		'PEN' => 'Sol',
		'PGK' => 'Kina papuana',
		'PHP' => 'Peso filippino',
		'PKR' => 'Rupia pakistana',
		'PLN' => 'Złoty polacco',
		'PTE' => 'Scudo portoghese',
		'PYG' => 'Guarani paraguaiano',
		'QAR' => 'Rial del Qatar',
		'ROL' => 'Vecchio leu rumeno',
		'RON' => 'Nuovo leu rumeno',
		'RSD' => 'Dinaro serbo',
		'RUB' => 'Rublo russo',
		'RUR' => 'Rublo russo',
		'RWF' => 'Franco ruandese',
		'SAR' => 'Rial saudita',
		'SBD' => 'Dollaro delle Salomone',
		'SCR' => 'Rupia delle Seychelles',
		'SDD' => 'Dinaro sudanese',
		'SDG' => 'Sterlina sudanese',
		'SEK' => 'Corona svedese',
		'SGD' => 'Dollaro di Singapore',
		'SHP' => 'Sterlina di Sant’Elena',
		'SIT' => 'Tolar',
		'SKK' => 'Corona slovacca',
		'SLE' => 'Leone',
		'SLL' => 'Leone sierraleonese',
		'SOS' => 'Scellino somalo',
		'SRD' => 'Dollaro surinamese',
		'SRG' => 'Dollaro surinamese',
		'STD' => '(Vecchia) Dobra di São Tomé e Príncipe',
		'STN' => 'Dobra',
		'SVC' => 'Colón salvadoregno',
		'SYP' => 'Sterlina siriana',
		'SZL' => 'Lilangeni dello Swaziland',
		'THB' => 'Baht thailandese',
		'TJS' => 'Somoni tagico',
		'TMM' => 'Manat turkmeno',
		'TMT' => 'Manat turkmeno',
		'TND' => 'Dinaro tunisino',
		'TOP' => 'Pa’anga tongano',
		'TPE' => 'Scudo di Timor',
		'TRL' => 'Lira turca (vecchia)',
		'TRY' => 'Nuova lira turca',
		'TTD' => 'Dollaro di Trinidad e Tobago',
		'TWD' => 'Nuovo dollaro taiwanese',
		'TZS' => 'Scellino tanzaniano',
		'UAH' => 'Grivnia ucraina',
		'UGX' => 'Scellino ugandese',
		'USD' => 'Dollaro statunitense',
		'UYU' => 'Peso uruguaiano',
		'UZS' => 'Som uzbeco',
		'VEB' => 'Bolivar',
		'VEF' => 'Bolivar',
		'VES' => 'Bolivar sovrano',
		'VND' => 'Dong vietnamita',
		'VUV' => 'Vatu di Vanuatu',
		'WST' => 'Tala samoano',
		'XAF' => 'Franco CFA BEAC',
		'XCD' => 'Dollaro dei Caraibi Orientali',
		'XCG' => 'Fiorino caraibico',
		'XOF' => 'Franco CFA BCEAO',
		'XPF' => 'Franco CFP',
		'YER' => 'Rial yemenita',
		'YUM' => 'Dinaro Jugoslavo',
		'ZAR' => 'Rand sudafricano',
		'ZMK' => 'Kwacha',
		'ZMW' => 'Kwacha zambiano',
		'ZWD' => 'Dollaro zimbabwiano',
		'ZWG' => 'Zimbabwe Gold',
		'ZWL' => 'Dollaro zimbabwiano',
	];

	/**
	 * Code List 96 for nb
	 *
	 * @var array
	 * @see https://ns.editeur.org/onix/nb/96
	 */
	protected static $nb = [
		'AED' => 'Emiratarabisk dirham',
		'AFA' => 'Afghani',
		'AFN' => 'Afghani',
		'ALL' => 'Albanske lek',
		'AMD' => 'Armenske dram',
		'ANG' => 'Antilliansk gylden',
		'AOA' => 'Angolansk kwanza',
		'ARS' => 'Argentinsk peso',
		'ATS' => 'Østerikske schilling',
		'AUD' => 'Australske dollar',
		'AWG' => 'Arubansk florin',
		'AZN' => 'Aserbajdsjansk manat',
		'BAM' => 'Konvertibel mark',
		'BBD' => 'Barbadisk dollar',
		'BDT' => 'Bangladeshisk taka',
		'BEF' => 'Belgiske franc',
		'BGL' => 'Bulgarske lev',
		'BGN' => 'Bulgarske lev',
		'BHD' => 'Bahrainsk dinar',
		'BIF' => 'Burundisk franc',
		'BMD' => 'Bermudisk dollar',
		'BND' => 'Bruneisk dollar',
		'BOB' => 'Boliviano',
		'BRL' => 'Brasiliansk real',
		'BSD' => 'Bahamansk dollar',
		'BTN' => 'Ngultrum',
		'BWP' => 'Pula',
		'BYR' => 'Hviterussiske rubel',
		'BYN' => 'Hviterussiske rubel',
		'BZD' => 'Belizisk dollar',
		'CAD' => 'Kanadisk dollar',
		'CDF' => 'Kongolesiske Franc',
		'CHF' => 'Sveitserfranc',
		'CLP' => 'Chilensk peso',
		'CNY' => 'Yuan Renminbi',
		'COP' => 'Colombiansk peso',
		'CRC' => 'Costaricansk colón',
		'CSD' => 'Serbisk dinar',
		'CUC' => 'Cubansk konvertibel peso',
		'CUP' => 'Cubansk peso',
		'CVE' => 'Kappverdisk escudo',
		'CYP' => 'Kypriotisk pund',
		'CZK' => 'Tsjekkisk koruna',
		'DEM' => 'Tyske mark',
		'DJF' => 'Djiboutisk franc',
		'DKK' => 'Danske kroner',
		'DOP' => 'Dominikansk peso',
		'DZD' => 'Algerisk dinar',
		'EEK' => 'Kroon',
		'EGP' => 'Egyptisk pund',
		'ERN' => 'Eritreisk nakfa',
		'ESP' => 'Spansk peseta',
		'ETB' => 'Etiopiske birr',
		'EUR' => 'Euro',
		'FIM' => 'Finske mark',
		'FJD' => 'Fijiansk dollar',
		'FKP' => 'Falklandspund',
		'FRF' => 'Franske franc',
		'GBP' => 'Britiske pund',
		'GEL' => 'Lari',
		'GHC' => 'Ghanesisk cedi',
		'GHS' => 'Cedi',
		'GIP' => 'Gibraltarsk pund',
		'GMD' => 'Gambisk dalasi',
		'GNF' => 'Guineansk franc',
		'GRD' => 'Greske drakmer',
		'GTQ' => 'Quetzal',
		'GWP' => 'Guinea-Bissau Peso',
		'GYD' => 'Guyansk dollar',
		'HKD' => 'Hong Kong Dollar',
		'HNL' => 'Lempira',
		'HRK' => 'Kroatisk kuna',
		'HTG' => 'Haitisk gourde',
		'HUF' => 'Forint',
		'IDR' => 'Rupiah',
		'IEP' => 'Irske pund',
		'ILS' => 'Israelsk shekel',
		'INR' => 'Indisk rupi',
		'IQD' => 'Irakisk dinar',
		'IRR' => 'Iransk rial',
		'ISK' => 'Islandsk krona',
		'ITL' => 'Italienske lire',
		'JMD' => 'Jamaicansk dollar',
		'JOD' => 'Jordansk dinar',
		'JPY' => 'Yen',
		'KES' => 'Kenyansk shilling',
		'KGS' => 'Som',
		'KHR' => 'Riel',
		'KMF' => 'Komorisk franc',
		'KPW' => 'Nordkoreansk won',
		'KRW' => 'Sørkoreansk won',
		'KWD' => 'Kuwaitisk dinar',
		'KYD' => 'Caymansk dollar',
		'KZT' => 'Tenge',
		'LAK' => 'Laotiske Kip',
		'LBP' => 'Libanesisk pund',
		'LKR' => 'Srilankisk rupi',
		'LRD' => 'Liberiansk dollar',
		'LSL' => 'Loti',
		'LTL' => 'Litauisk litas',
		'LUF' => 'Luxemburg franc',
		'LVL' => 'Latvisk lat',
		'LYD' => 'Libysk dinar',
		'MAD' => 'Marokkansk dirham',
		'MDL' => 'Moldovisk leu',
		'MGA' => 'Ariary',
		'MGF' => 'Gassisk franc',
		'MKD' => 'Denar',
		'MMK' => 'Kyat',
		'MNT' => 'Tugrik',
		'MOP' => 'Pataca',
		'MRO' => '(Old) Ouguiya',
		'MRU' => 'Ouguiya',
		'MTL' => 'Maltesisk lira',
		'MUR' => 'Mauritisk rupi',
		'MVR' => 'Maldivisk rufiyaa',
		'MWK' => 'Malawisk kwacha',
		'MXN' => 'Mexikansk peso',
		'MYR' => 'Ringgit',
		'MZN' => 'Metical',
		'NAD' => 'Namibisk dollar',
		'NGN' => 'Naira',
		'NIO' => 'Córdoba',
		'NLG' => 'Nederlandsk gylden',
		'NOK' => 'Norsk krone',
		'NPR' => 'Nepalsk rupi',
		'NZD' => 'Newzealandsk dollar',
		'OMR' => 'Omansk rial',
		'PAB' => 'Balboa',
		'PEN' => 'Sol',
		'PGK' => 'Kina',
		'PHP' => 'Filippinsk peso',
		'PKR' => 'Pakistansk rupi',
		'PLN' => 'Polsk zloty',
		'PTE' => 'Portugisiske escudo',
		'PYG' => 'Guarani',
		'QAR' => 'Qatarsk riyal',
		'ROL' => 'Rumensk leu',
		'RON' => 'Ny rumensk Leu',
		'RSD' => 'Serbisk dinar',
		'RUB' => 'Russisk rubel',
		'RUR' => 'Russisk rubel',
		'RWF' => 'Rwandisk franc',
		'SAR' => 'Saudiarabisk riyal',
		'SBD' => 'Salomonsk dollar',
		'SCR' => 'Seychellisk rupi',
		'SDD' => 'Sudanesisk dinar',
		'SDG' => 'Sudanesisk pund',
		'SEK' => 'Svensk krona',
		'SGD' => 'Singaporsk dollar',
		'SHP' => 'Sankthelensk pund',
		'SIT' => 'Tolar',
		'SKK' => 'Slovakisk koruna',
		'SLE' => 'Leone',
		'SLL' => 'Leone',
		'SOS' => 'Somalisk shilling',
		'SRD' => 'Surinamsk dollar',
		'SRG' => 'Surinamsk gylden',
		'STD' => '(Old) Dobra',
		'STN' => 'Dobra',
		'SVC' => 'Salvadoransk colon',
		'SYP' => 'Syrisk pund',
		'SZL' => 'Lilangeni',
		'THB' => 'Baht',
		'TJS' => 'Tadsjikisk somoni',
		'TMM' => 'Turkmensk manat',
		'TMT' => 'Manat',
		'TND' => 'Tunisisk dinar',
		'TOP' => 'Pa’anga',
		'TPE' => 'Timor Escudo',
		'TRL' => 'Tyrkisk lira (gammel)',
		'TRY' => 'Tyrkisk lira (ny)',
		'TTD' => 'Trinidad and Tobago-dollar',
		'TWD' => 'Taiwansk dollar',
		'TZS' => 'Tanzaniansk shilling',
		'UAH' => 'Hryvnia',
		'UGX' => 'Uganda shilling',
		'USD' => 'Amerikanske dollar',
		'UYU' => 'Uruguayisk peso',
		'UZS' => 'Usbekisk sum',
		'VEB' => 'Bolivar',
		'VEF' => 'Bolivar',
		'VES' => 'Bolivar Soberano',
		'VND' => 'Vietamesisk dong',
		'VUV' => 'Vatu',
		'WST' => 'Tala',
		'XAF' => 'CFA franc BEAC',
		'XCD' => 'Østkaribisk dollar',
		'XCG' => 'Karibiske gylden',
		'XOF' => 'CFA franc BCEAO',
		'XPF' => 'CFP franc',
		'YER' => 'Jemenittisk rial',
		'YUM' => 'Jugoslavisk dinar',
		'ZAR' => 'Rand',
		'ZMK' => 'Kwacha',
		'ZMW' => 'Zambisk kwacha',
		'ZWD' => 'Zimbabwe dollar',
		'ZWG' => 'Zimbabwe Gold',
		'ZWL' => 'Zimbabwe dollar',
	];

	/**
	 * Code List 96 for tr
	 *
	 * @var array
	 * @see https://ns.editeur.org/onix/tr/96
	 */
	protected static $tr = [
		'AED' => 'BAE Dirhemi',
		'AFA' => 'Afgan',
		'AFN' => 'Afgan',
		'ALL' => 'Lek',
		'AMD' => 'Ermeni Dramı',
		'ANG' => 'Hollanda Antilleri guldeni',
		'AOA' => 'Kwanza',
		'ARS' => 'Arjantin Pesosu',
		'ATS' => 'Avusturya, Şilin',
		'AUD' => 'Avustralya Doları',
		'AWG' => 'Aruba Florini',
		'AZN' => 'Azerbaycan Manatı',
		'BAM' => 'Konvertible Mark',
		'BBD' => 'Barbados Doları',
		'BDT' => 'Taka',
		'BEF' => 'Belçika Frangı',
		'BGL' => 'Lev',
		'BGN' => 'Lev',
		'BHD' => 'Bahreyn Dinarı',
		'BIF' => 'Burundi Frangı',
		'BMD' => 'Bermuda Doları',
		'BND' => 'Brunei Dolar',
		'BOB' => 'Boliviano',
		'BRL' => 'Brezilya Reali',
		'BSD' => 'Bahama Doları',
		'BTN' => 'Ngultrun',
		'BWP' => 'Pula',
		'BYR' => 'Belarus rublesi',
		'BYN' => 'Beyaz Rus Rublesi',
		'BZD' => 'Belize Doları',
		'CAD' => 'Kanada Doları',
		'CDF' => 'Kongo Frangı',
		'CHF' => 'İsviçre Frangı',
		'CLP' => 'Şili Pesosu',
		'CNY' => 'Yuan Renminbi',
		'COP' => 'Kolombiya Pesosu',
		'CRC' => 'Kosta Rika Kolonu',
		'CSD' => 'Sırp Dinarı',
		'CUC' => 'Küba Konvertible Pesosu',
		'CUP' => 'Küba Pesosu',
		'CVE' => 'Yeşil Burun Adaları eskudosu',
		'CYP' => 'Kıbrıs Lirası',
		'CZK' => 'Çek Korunası',
		'DEM' => 'Alman Markı',
		'DJF' => 'Cibuti Frangı',
		'DKK' => 'Danimarka Kronu',
		'DOP' => 'Dominik Pesosu',
		'DZD' => 'Cezayir Dinarı',
		'EEK' => 'Kroon',
		'EGP' => 'Mısır Lirası',
		'ERN' => 'Nakfa',
		'ESP' => 'İspanyol Pesetası',
		'ETB' => 'Etiyopya Birri',
		'EUR' => 'Euro',
		'FIM' => 'Finlandiya Markası',
		'FJD' => 'Fiji Doları',
		'FKP' => 'Falkland Adaları Lirası',
		'FRF' => 'Fransız Frangı',
		'GBP' => 'İngiliz Sterlini / İngiliz Lirası',
		'GEL' => 'Lari',
		'GHC' => 'Cedi',
		'GHS' => 'Cedi',
		'GIP' => 'Cebelitarık Lirası',
		'GMD' => 'Dalasi',
		'GNF' => 'Gine Frangı',
		'GRD' => 'Yunan Drahmisi',
		'GTQ' => 'Kuetzal',
		'GWP' => 'Gine-Bissau pezosu',
		'GYD' => 'Guyana Doları',
		'HKD' => 'Hong Kong Doları',
		'HNL' => 'Lempira',
		'HRK' => 'Hırvat Kunası',
		'HTG' => 'Gourde',
		'HUF' => 'Forint',
		'IDR' => 'Rupi',
		'IEP' => 'İrlanda, Punt',
		'ILS' => 'İsrail şekeli',
		'INR' => 'Hint Rupisi',
		'IQD' => 'Irak Dinarı',
		'IRR' => 'İran Riyali',
		'ISK' => 'İzlanda Kronu',
		'ITL' => 'İtalya, Lira',
		'JMD' => 'Jamaika Doları',
		'JOD' => 'Ürdün Dinarı',
		'JPY' => 'Yen',
		'KES' => 'Kenya Şilini',
		'KGS' => 'Som',
		'KHR' => 'Riel',
		'KMF' => 'Komor Frangı',
		'KPW' => 'Kuzey Kore Wonu',
		'KRW' => 'Won',
		'KWD' => 'Kuveyt Dinarı',
		'KYD' => 'Cayman Adaları Doları',
		'KZT' => 'Tenge',
		'LAK' => 'Kip',
		'LBP' => 'Lübnan Lirası',
		'LKR' => 'Sri Lanka Rupisi',
		'LRD' => 'Liberya Doları',
		'LSL' => 'Loti',
		'LTL' => 'Litvanya Litusu',
		'LUF' => 'Lüksemburg Frangı',
		'LVL' => 'Letonya Latı',
		'LYD' => 'Libya Dinarı',
		'MAD' => 'Fas Dirhemi',
		'MDL' => 'Moldova Leyi',
		'MGA' => 'Ariary',
		'MGF' => 'Madagaskar Frangı',
		'MKD' => 'Denar',
		'MMK' => 'Kyatı',
		'MNT' => 'Tugrik',
		'MOP' => 'Pataca',
		'MRO' => 'Ouguiya',
		'MRU' => 'Ouguiya',
		'MTL' => 'Malta Lirası',
		'MUR' => 'Mauritius Rupisi',
		'MVR' => 'Rufiyaa',
		'MWK' => 'Kwacha',
		'MXN' => 'Meksika Pesosu',
		'MYR' => 'Malezya Ringiti',
		'MZN' => 'Metikali',
		'NAD' => 'Namibya Doları',
		'NGN' => 'Naira',
		'NIO' => 'Cordoba Oro',
		'NLG' => 'Hollanda Florini',
		'NOK' => 'Norveç Kronu',
		'NPR' => 'Nepal Rupisi',
		'NZD' => 'Yeni Zelanda Doları',
		'OMR' => 'Umman Riyali',
		'PAB' => 'Balboa',
		'PEN' => 'Nuevo Sol',
		'PGK' => 'Kina',
		'PHP' => 'Filipin Pesosu',
		'PKR' => 'Pakistan Rupisi',
		'PLN' => 'Zloti',
		'PTE' => 'Portekiz, Escudo',
		'PYG' => 'Guarani',
		'QAR' => 'Katar Riyali',
		'ROL' => 'Eski Ley',
		'RON' => 'Yeni Ley',
		'RSD' => 'Sırp Dinarı',
		'RUB' => 'Rus Rublesi',
		'RUR' => 'Rus Rublesi',
		'RWF' => 'Ruanda Frangı',
		'SAR' => 'Suudi Arabistan Riyali',
		'SBD' => 'Solomon Adaları Doları',
		'SCR' => 'Seyşeller Rupisi',
		'SDD' => 'Sudan Dinarı',
		'SDG' => 'Sudan Lirası',
		'SEK' => 'İsveç Kronu',
		'SGD' => 'Singapur Doları',
		'SHP' => 'Saint Helena Lirası',
		'SIT' => 'Tolar',
		'SKK' => 'Slovak Korunası',
		'SLE' => 'Leone',
		'SLL' => 'Leone',
		'SOS' => 'Somali Şilini',
		'SRD' => 'Surinam Florini',
		'SRG' => 'Surinam Florini',
		'STD' => 'Dobra',
		'STN' => '(Yeni) Dobra',
		'SVC' => 'El Salvador Kolonu',
		'SYP' => 'Suriye Lirası',
		'SZL' => 'Lilangeni',
		'THB' => 'Baht',
		'TJS' => 'Somoni',
		'TMM' => 'Manat',
		'TMT' => 'Manat',
		'TND' => 'Tunus Dinarı',
		'TOP' => 'Pa’anga',
		'TPE' => 'Timor Escudo',
		'TRL' => 'Türk Lirası (eski)',
		'TRY' => 'Türk Lirası (yeni)',
		'TTD' => 'Trinidad ve Tobago Doları',
		'TWD' => 'Yeni Tayvan Doları',
		'TZS' => 'Tanzanya Şilini',
		'UAH' => 'Grivna',
		'UGX' => 'Uganda Şilini',
		'USD' => 'ABD Doları',
		'UYU' => 'Uruguay Pesosu',
		'UZS' => 'Özbekistan Sumu',
		'VEB' => 'Bolivar',
		'VEF' => 'Bolivar fuerte',
		'VES' => 'Bolívar Soberano',
		'VND' => 'Dong',
		'VUV' => 'Vatu',
		'WST' => 'Tala',
		'XAF' => 'CFA Frangı BEAC',
		'XCD' => 'Doğu Karayip Doları',
		'XCG' => 'Caribbean Guilder',
		'XOF' => 'CFA Frangı BCEAO',
		'XPF' => 'CFP Frangı',
		'YER' => 'Yemen Riyali',
		'YUM' => 'Yugoslav Dinarı',
		'ZAR' => 'Rand',
		'ZMK' => 'Kwacha',
		'ZMW' => 'Zambiya Kwachası',
		'ZWD' => 'Zimbabve Doları',
		'ZWG' => 'Zimbabwe Gold',
		'ZWL' => 'Zimbabve Doları',
	];
}
